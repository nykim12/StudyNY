...
//	매일 새벽 3시에 어제 첨부된 파일 중 잘못된 파일들을 찾아서 제거
	@Scheduled(cron = "0 0/1 * * * *") // 새벽 3시 동작("0 0 3 * * *"), 실제로는 1분마다 동작
...
//		어제 저장된 첨부 파일 리스트
		List<FileAttach> fileAttaches = galleryMapper.selectFileAttachListAtYesterday();

//		어제 저장된 첨부 파일들을 Path 객체(경로 + 파일명)로 List에 저장
...
//		어제 경로 디렉터리에 실제로 저장된 파일들과 DB에서 가져온 어제 저장된 첨부 파일 내역을 비교하여 일치하지 않는 파일을 제거
