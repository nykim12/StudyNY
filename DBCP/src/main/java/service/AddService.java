...
public class AddService implements EmpService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		if (res > 0) {
			...
			/*
			 * 1. new ActionForward("emp/list.jsp", true); DB의 목록을 가져오지 않고 list.jsp로 그냥 이동하기
			 * 때문에 보여지는 내용이 없다. 2. new ActionForward("/DBCP/list.do", true); list.do 매핑은
			 * ListService를 실행시키는 매핑이다. ListService가 실행되면 DB의 목록을 가져와서 request에 속성으로 저장하고,
