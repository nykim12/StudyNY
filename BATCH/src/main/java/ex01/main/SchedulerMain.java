...
public class SchedulerMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			JobDetail job = JobBuilder.newJob(HelloJob.class).withIdentity("job1", "group1") // 개발자가 정하면 됨.
					.build();

			// Scheduler 인스턴스가 동작할 시점 생성(SimpleScheduleBuilder)
			Trigger trigger = TriggerBuilder.newTrigger().withIdentity("trigger1", "group1") // 이름 trigger1(개발자 마음대로),
																								// 그룹 group1은 Job의 그룹과
																								// 맞추기
					.startNow()
